...
function shouldLibraryBeImported(
  ...
  const shouldNpmLibraryBeImported =
    typesLibraryName === null &&
    isLibraryAllowed(npmLibraryName, importedLibraries);

  // library from @types can be imported only when it is specified explicitly
  const shouldTypesLibraryBeImported =
    importedLibraries !== undefined &&
    typesLibraryName !== null &&
    isLibraryAllowed(typesLibraryName, importedLibraries);
